...
int runEncode(const char* rfilename, const char* wfilename) {//need to add padding
...
char** getBitstrings(Tree* tree) {
    char** array = doMalloc((CHAR_MAX + 1) * CHAR_BIT * sizeof(char));
    ...
    char* bitstring = doMalloc(CHAR_BIT * sizeof(char));
...
void recGetBitstrings(Node* node, char** bitstrings, char* bitstring, int level) {
    if (node->left == NULL) {
        bitstring[level] = '\0';
        bitstrings[node->c] = doMalloc(CHAR_BIT * sizeof(char));
...
void encodeFile(char** bitstrings, FILE* rfile, FILE* wfile) {
    ...
    fprintf(stderr, "%d total bits were printed\n", bit_counter); //for debugging
}